                    ...
                    <div class="card">
                        <div class="m-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text"value="{{ old('app_area', $product->app_area) }}" name="app_area"
                                    class="form-control" placeholder="Enter application area">
                            ...
                            {{-- <div class="row" id="size-container">
                                <div class="col-md-4">
                                    ...
                                    <input type="number"
                                        value="{{ old('length', optional($product->sizes->first())->length ?? 0) }}"
                                        name="length[]" class="form-control" value="00" min="0">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Width</label>
                                    <input type="number"
                                        value="{{ old('width', optional($product->sizes->first())->width ?? 0) }}"
                                        name="width[]" class="form-control" value="00" min="0">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">Thickness</label>
                                    <input type="number"
                                        value="{{ old('thickness', optional($product->sizes->first())->thickness ?? 0) }}"
                                        name="thickness[]" class="form-control" value="00" min="0">
                                </div>
                            </div> --}}
                            <div class="row" id="size-container">
                                @php
                                    $predefinedKeys = ['Length', 'Width', 'Thickness'];
                                    $sizes = $product->sizes ?? collect();

                                    // Normalize keys to avoid case-sensitive duplicates
                                    $sizesGrouped = $sizes->groupBy(fn($item) => strtolower($item->key));
                                @endphp
                                ...
                                @foreach ($predefinedKeys as $key)
                                    @php
                                        ...
                                        $size = $sizesGrouped->get($normalizedKey)?->first();
                                    @endphp
                                    <div class="col-md-4">
                                        <label class="form-label">{{ $key }}</label>
                                        <input type="number" value="{{ old("sizes.$key", optional($size)->value ?? 0) }}"
                                            name="sizes[{{ $key }}]" class="form-control" min="0">
                                        <input type="hidden" name="units[{{ $key }}]"
                                            value="{{ optional($size)->unit ?? 'ft' }}">
                                    </div>
                                @endforeach

                                {{-- Custom Sizes --}}
                                @foreach ($sizesGrouped as $key => $group)
                                    @if (!in_array(ucfirst($key), $predefinedKeys))
                                        @php $size = $group->first(); @endphp
                                        <div class="col-md-4 custom-size">
                                            <label class="form-label">{{ ucfirst($size->key) }}</label>
                                            <input type="hidden" name="custom_keys[]" value="{{ $size->key }}"
                                                class="form-control" placeholder="Custom Key">
                                            <input type="number" name="custom_values[]" value="{{ $size->value }}"
                                                class="form-control" min="0">

                                            <button type="button" class="btn btn-danger remove-size mt-2"> X </button>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="gst" value="{{ old('gst', $product->gst ?? 0) }}"
                                    class="form-control" placeholder="Enter GST Percentage">
                            </div>
                            <div class="mb-3">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <input type="number" name="warranty_duration"
                                            value="{{ old('warranty_duration', $product->warranty_duration ?? 0) }}"
                        ...
                        <div class="m-3">
                            ...
                            <select name="adhesive_id" class="form-select mb-3">
                                ...
                                @foreach ($adhesives as $adhesive)
                                    <option value="{{ $adhesive->id }}"
                                        {{ old('category_id', $product->adhesive_id) == $adhesive->id ? 'selected' : '' }}>
                                        {{ $adhesive->name }}</option>
                                @endforeach
                            </select>
                            <div class="mb-3">
                                <label class="form-label">Labor Charges (per sq.ft)</label>
                                <input type="text" name="labor_charges"
                                    value="{{ old('labor_charge', $product->labor_charge ?? 0) }}"class="form-control"
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-6">
                                    <input type="number" name="estimated_delivery_time"
                                        value="{{ old('estimated_delivery_time', $product->estimate_delivery_duration ?? 0) }}"
                                        class="form-control" placeholder="Duration" min="0">
                                </div>
                                <div class="col-md-6">
                                    <select name="estimated_delivery_type" class="form-select">
                                        <option value="days"
                                            {{ old('estimated_delivery_type', $product->estimated_delivery_type ?? '') == 'days' ? 'selected' : '' }}>
                                            Days</option>
                                        <option value="months"
                                            {{ old('estimated_delivery_type', $product->estimated_delivery_type ?? '') == 'months' ? 'selected' : '' }}>
                                            Months</option>
                        ...
                        <div class="card m-2">
                            <div class="m-3">
                                ...
                                <button class="btn btn-dark w-100 mb-3" data-bs-toggle="modal"
                                    data-bs-target="#addproduct">Add Product Images</button>
                                                ...
                                                <div style="display:flex;flex-direction:row;gap:5%;width:100% ;">
                                                    <div class="column">
                                                        <div class="mb-3">
                                                            ...
                                                            <div class="border p-3 text-center"
                                                                ...
                                                                <label for="productImage"
                                                                    class="d-block cursor-pointer">📤 Drag your file(s) or
                                                                    <a href="#">browse</a></label>
                                                                <small class="text-muted">Max 10 MB files are
                                                                    allowed</small>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="text" class="form-control" id="pdfName"
                                                                placeholder="Enter PDF Name">
                                                            <span class="text-danger d-none"
                                                                id="pdfNameError">Required</span>
                                                        </div>
                                                        <div class="mb-3">
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="productColor" class="form-label">Product
                                                                Color</label>
                                                    ...
                                                    <div class="column">
                                                        <div class="row">
                                                            <div class="col-md-6"
                                                                ...
                                                                <label for="purchaseCost" class="form-label">Purchase
                                                                    cost*</label>
                                                                <input type="number" class="form-control"
                                                                    id="purchaseCost" name="purchase_cost" value="0"
                                                                    min="0" placeholder="₹ 00" required>
                                                                <span class="text-danger d-none"
                                                                    id="purchaseCostError">Required</span>
                                                            </div>
                                                            <div class="col-md-6"
                                                                style="display:flex;flex-direction:column">
                                                                <label for="sellingPrice" class="form-label">Selling
                                                                    price*</label>
                                                                <input type="number" class="form-control"
                                                                    id="sellingPrice" name="selling_price" value="0"
                                                                    min="0" placeholder="₹ 00" required>
                                                                <span class="text-danger d-none"
                                                                    id="sellingPriceError">Required</span>
                                                            </div>
                                                        </div>
                                                        <div class="mb-3">
                                                        ...
                                                        <div class="form-check">
                                                            ...
                                                            <label class="form-check-label" for="stockAvailable">Stock
                                                                Available</label>
                                                            <input class="form-check-input" type="checkbox"
                                                                id="stockAvailable" checked>
                                                        </div>
                            ...
                            <div class="container mt-4">
                                                ...
                                                <tr>
                                                    <td>
                                                        @if ($product->images->isNotEmpty())
                                                            <img src="{{ asset($p->product_image) }}" alt="Product Image"
                                                                class="img-fluid"
                                ...
                                <div id="uploadSection" class="border p-3 text-center rounded"
                                    style="border-style: dashed; cursor: pointer;">
                                    <p class="mb-1">Drag your file(s) here or <a href="#"
                                            onclick="document.getElementById('imageUpload').click(); return false;">browse</a>
                                    </p>
                                    <p class="text-muted">Max 10 MB files are allowed</p>
                                    <input type="file" id="imageUpload" class="d-none" multiple accept="image/*">
                                </div>

                                <!-- Image Preview Sectio
                                                    @en -->
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll(".edit-row").forEach(button => {
                button.addEventListener("click", function () {
                    ...
                    if (imageId) {
                        window.location.href = `/edit-product-image/${imageId}/${productId}`; // Redirect to the edit page
                    }
                });
            });
